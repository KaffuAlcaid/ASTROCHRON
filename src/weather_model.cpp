...
WeatherModel::WeatherModel(QObject *parent) : QObject(parent)
{
    ...
    m_status = m_enabled ? QStringLiteral("天气待获取") : QStringLiteral("天气查询已关闭");
...
void WeatherModel::setEnabled(bool enabled)
{
    ...
    if (enabled) { m_refreshTimer.start(); refresh(); }
        ...
        m_hourly = {}; m_fetchedAt = {};
        m_status = QStringLiteral("天气查询已关闭");
    }
...
void WeatherModel::refresh()
{
    ...
    m_busy = true; m_status = QStringLiteral("正在获取天气预报"); emit statusChanged();
    connect(reply, &QNetworkReply::finished, this, [this, reply, request] {
        ...
        if (reply->error() == QNetworkReply::NoError && valid) {
            ...
            m_status = QStringLiteral("逐小时预报");
            emit forecastChanged();
        } else m_status = reply->error() == QNetworkReply::NoError ? QStringLiteral("天气预报暂缺") : QStringLiteral("天气获取失败：") + reply->errorString();
        emit statusChanged();
    });
}
